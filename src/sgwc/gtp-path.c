...
static void _gtpv1_u_recv_cb(short when, ogs_socket_t fd, void *data)
{
    ...
    if (gtp_h->type == OGS_GTPU_MSGTYPE_ECHO_REQ) {
        ...
        tunnel = sgwc_tunnel_find_by_teid(teid);
        if (!tunnel) {
            if (gtp_h->type == OGS_GTPU_MSGTYPE_GPDU)
        ...
        if (tunnel->interface_type == OGS_GTP_F_TEID_S1_U_SGW_GTP_U) {
            ...
            sgwc_tunnel_t *indirect_tunnel = NULL;

            indirect_tunnel = sgwc_tunnel_find_by_interface_type(bearer,
                    tunnel->interface_type);
            ...
            sgwc_tunnel_t *s1u_tunnel = NULL;

            s1u_tunnel = sgwc_s1u_tunnel_in_bearer(bearer);
            ...
            if (s1u_tunnel->remote_teid) {
                ...
                ogs_assert(bearer->sess->sgwc_ue);

                sgwc_ue = bearer->sess->sgwc_ue;

                ogs_debug("[SGW] S1U PATH deactivated : STATE[0x%x]",
                        SGW_GET_UE_STATE(sgwc_ue));
                if ((SGW_GET_UE_STATE(sgwc_ue) & SGW_S1U_INACTIVE)) {
                    ogs_debug("    SGW-S1U Inactive");
                    if (!(SGW_GET_UE_STATE(sgwc_ue) & SGW_DL_NOTI_SENT)) {
                        sgwc_event_t *e;
                        ...
                        SGW_SET_UE_STATE(sgwc_ue, SGW_DL_NOTI_SENT);
                    }
...
int sgwc_gtp_open(void)
{
...
#if 0
    ogs_list_for_each(&sgwc_self()->gtpu_list, node) {
        sock = ogs_gtp_server(node);
        ogs_assert(sock);
    ...
    ogs_list_for_each(&sgwc_self()->gtpu_list6, node) {
        sock = ogs_gtp_server(node);
        ogs_assert(sock);
    ...
    sgwc_self()->gtpu_sock = ogs_socknode_sock_first(&sgwc_self()->gtpu_list);
    if (sgwc_self()->gtpu_sock)
        sgwc_self()->gtpu_addr = &sgwc_self()->gtpu_sock->local_addr;

    sgwc_self()->gtpu_sock6 = ogs_socknode_sock_first(&sgwc_self()->gtpu_list6);
    if (sgwc_self()->gtpu_sock6)
        sgwc_self()->gtpu_addr6 = &sgwc_self()->gtpu_sock6->local_addr;

    ogs_assert(sgwc_self()->gtpu_addr || sgwc_self()->gtpu_addr6);
#endif
...
#if 0
    ogs_socknode_remove_all(&sgwc_self()->gtpu_list);
    ogs_socknode_remove_all(&sgwc_self()->gtpu_list6);
    ogs_socknode_remove_all(&sgwc_self()->adv_gtpu_list);
    ogs_socknode_remove_all(&sgwc_self()->adv_gtpu_list6);
#endif
