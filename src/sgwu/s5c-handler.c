...
void sgwu_s5c_handle_create_session_response(ogs_gtp_xact_t *s5c_xact, 
    sgwu_sess_t *sess, ogs_gtp_message_t *message)
{
    int rv;
    uint8_t cause_value;
    ...
    ogs_gtp_f_teid_t sgwu_s11_teid;
    ogs_gtp_f_teid_t *pgw_s5u_teid = NULL;
    ogs_gtp_f_teid_t sgwu_s1u_teid;
    int len;
    ...
    if (cause_value == OGS_GTP_CAUSE_REQUEST_ACCEPTED) {
        bearer = sgwu_bearer_find_by_sess_ebi(sess,
    ...
    if (cause_value != OGS_GTP_CAUSE_REQUEST_ACCEPTED) {
        ogs_gtp_send_error_message(s11_xact, sgwu_ue ? sgwu_ue->mme_s11_teid : 0,
                OGS_GTP_CREATE_SESSION_RESPONSE_TYPE,
                cause_value);
        return;
    }
    ...
    pgw = ogs_gtp_node_find_by_f_teid(&sgwu_self()->pgw_s5u_list, pgw_s5u_teid);
    if (!pgw) {
        pgw = ogs_gtp_node_add_by_f_teid(
            &sgwu_self()->pgw_s5u_list, pgw_s5u_teid, sgwu_self()->gtpu_port,
        ...
        rv = ogs_gtp_connect(
                sgwu_self()->gtpu_sock, sgwu_self()->gtpu_sock6, pgw);
    ...
    memset(&sgwu_s11_teid, 0, sizeof(ogs_gtp_f_teid_t));
    sgwu_s11_teid.interface_type = OGS_GTP_F_TEID_S11_S4_SGW_GTP_C;
    sgwu_s11_teid.teid = htonl(sgwu_ue->sgwu_s11_teid);
    rv = ogs_gtp_sockaddr_to_f_teid(
            sgwu_self()->gtpc_addr, sgwu_self()->gtpc_addr6, &sgwu_s11_teid, &len);
    ogs_assert(rv == OGS_OK);
    rsp->sender_f_teid_for_control_plane.presence = 1;
    rsp->sender_f_teid_for_control_plane.data = &sgwu_s11_teid;
    rsp->sender_f_teid_for_control_plane.len = len;

    /* Send Data Plane(UL) : SGW-S1U */
    memset(&sgwu_s1u_teid, 0, sizeof(ogs_gtp_f_teid_t));
    sgwu_s1u_teid.interface_type = s1u_tunnel->interface_type;
    sgwu_s1u_teid.teid = htonl(s1u_tunnel->local_teid);
    if (sgwu_self()->gtpu_addr) {
        addr = ogs_hash_get(sgwu_self()->adv_gtpu_hash,
                        &sgwu_self()->gtpu_addr->sin.sin_addr,
                        sizeof(sgwu_self()->gtpu_addr->sin.sin_addr));
    }
    if (sgwu_self()->gtpu_addr6) {
        addr6 = ogs_hash_get(sgwu_self()->adv_gtpu_hash6,
                        &sgwu_self()->gtpu_addr6->sin6.sin6_addr,
                        sizeof(sgwu_self()->gtpu_addr6->sin6.sin6_addr));
    }
    // Swap the SGW-S1U IP to IP to be advertised to UE
    if (addr || addr6) {
        rv = ogs_gtp_sockaddr_to_f_teid(addr,  addr6, &sgwu_s1u_teid, &len);
        ogs_assert(rv == OGS_OK);
    } else {
        rv = ogs_gtp_sockaddr_to_f_teid(
        sgwu_self()->gtpu_addr,  sgwu_self()->gtpu_addr6, &sgwu_s1u_teid, &len);
        ogs_assert(rv == OGS_OK);
    }
    rsp->bearer_contexts_created.s1_u_enodeb_f_teid.presence = 1;
    rsp->bearer_contexts_created.s1_u_enodeb_f_teid.data = &sgwu_s1u_teid;
...
void sgwu_s5c_handle_delete_session_response(ogs_gtp_xact_t *s5c_xact,
    sgwu_sess_t *sess, ogs_gtp_message_t *message)
{
    int rv;
    uint8_t cause_value;
...
void sgwu_s5c_handle_create_bearer_request(ogs_gtp_xact_t *s5c_xact, 
    sgwu_sess_t *sess, ogs_gtp_message_t *message)
{
    int rv;
    ...
    req->bearer_contexts.s1_u_enodeb_f_teid.data = &sgwu_s1u_teid;
    ...
    s11_xact = ogs_gtp_xact_local_create(
            sgwu_ue->gnode, &message->h, pkbuf, timeout, sess);
...
void sgwu_s5c_handle_update_bearer_request(ogs_gtp_xact_t *s5c_xact, 
    sgwu_sess_t *sess, ogs_gtp_message_t *message)
{
    int rv;
    uint8_t cause_value = 0;
...
void sgwu_s5c_handle_delete_bearer_request(ogs_gtp_xact_t *s5c_xact, 
    sgwu_sess_t *sess, ogs_gtp_message_t *message)
{
    int rv;
    uint8_t cause_value = 0;
...
void sgwu_s5c_handle_bearer_resource_failure_indication(ogs_gtp_xact_t *s5c_xact,
        sgwu_sess_t *sess, ogs_gtp_message_t *message)
{
    uint8_t cause_value = 0;
    ogs_gtp_xact_t *s11_xact = NULL;
