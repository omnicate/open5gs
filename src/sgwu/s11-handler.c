...
static void timeout(ogs_gtp_xact_t *xact, void *data)
{
    ...
    ogs_error("GTP Timeout : IMSI[%s] Message-Type[%d]",
            sgwu_ue->imsi_bcd, type);
}

void sgwu_s11_handle_create_session_request(ogs_gtp_xact_t *s11_xact,
        sgwu_ue_t *sgwu_ue, ogs_gtp_message_t *message)
{
    int rv;
    ...
    ogs_gtp_f_teid_t sgwu_s5c_teid, sgwu_s5u_teid;
    ogs_gtp_uli_t uli;
    ...
    if (cause_value != OGS_GTP_CAUSE_REQUEST_ACCEPTED) {
        ogs_gtp_send_error_message(
                s11_xact, sgwu_ue ? sgwu_ue->mme_s11_teid : 0,
    ...
    sess = sgwu_sess_find_by_ebi(sgwu_ue,
            req->bearer_contexts_to_be_created.eps_bearer_id.u8);
    if (sess) {
        ogs_warn("OLD Session Release [IMSI:%s,APN:%s]",
                sgwu_ue->imsi_bcd, sess->pdn.apn);
        sgwu_sess_remove(sess);
    }
    sess = sgwu_sess_add(sgwu_ue, apn,
            req->bearer_contexts_to_be_created.eps_bearer_id.u8);
    ogs_assert(sess);

    bearer = sgwu_default_bearer_in_sess(sess);
    ogs_assert(bearer);
    ...
    sgwu_ue->mme_s11_teid = ntohl(mme_s11_teid->teid);

    /* Send Control Plane(DL) : SGW-S5C */
    memset(&sgwu_s5c_teid, 0, sizeof(ogs_gtp_f_teid_t));
    sgwu_s5c_teid.interface_type = OGS_GTP_F_TEID_S5_S8_SGW_GTP_C;
    sgwu_s5c_teid.teid = htonl(sess->sgwu_s5c_teid);
    rv = ogs_gtp_sockaddr_to_f_teid(
        sgwu_self()->gtpc_addr, sgwu_self()->gtpc_addr6, &sgwu_s5c_teid, &len);
    ogs_assert(rv == OGS_OK);
    req->sender_f_teid_for_control_plane.presence = 1;
    req->sender_f_teid_for_control_plane.data = &sgwu_s5c_teid;
    req->sender_f_teid_for_control_plane.len = len;

    ogs_debug("    MME_S11_TEID[%d] SGW_S11_TEID[%d]",
        sgwu_ue->mme_s11_teid, sgwu_ue->sgwu_s11_teid);
    ogs_debug("    SGW_S5C_TEID[0x%x] PGW_S5C_TEID[0x%x]",
        sess->sgwu_s5c_teid, sess->pgw_s5c_teid);
    ...
    pgw = ogs_gtp_node_find_by_f_teid(&sgwu_self()->pgw_s5c_list, pgw_s5c_teid);
    if (!pgw) {
        pgw = ogs_gtp_node_add_by_f_teid(
            &sgwu_self()->pgw_s5c_list, pgw_s5c_teid, sgwu_self()->gtpc_port,
        ...
        rv = ogs_gtp_connect(
                sgwu_self()->gtpc_sock, sgwu_self()->gtpc_sock6, pgw);
        ogs_assert(rv == OGS_OK);
    }
    /* Setup GTP Node */
    ...
    memset(&sgwu_s5u_teid, 0, sizeof(ogs_gtp_f_teid_t));
    sgwu_s5u_teid.teid = htonl(s5u_tunnel->local_teid);
    sgwu_s5u_teid.interface_type = OGS_GTP_F_TEID_S5_S8_SGW_GTP_U;
    rv = ogs_gtp_sockaddr_to_f_teid(
        sgwu_self()->gtpu_addr,  sgwu_self()->gtpu_addr6, &sgwu_s5u_teid, &len);
    ogs_assert(rv == OGS_OK);
    req->bearer_contexts_to_be_created.s5_s8_u_sgw_f_teid.presence = 1;
    req->bearer_contexts_to_be_created.s5_s8_u_sgw_f_teid.data = &sgwu_s5u_teid;
...
void sgwu_s11_handle_modify_bearer_request(ogs_gtp_xact_t *s11_xact,
    sgwu_ue_t *sgwu_ue, ogs_gtp_modify_bearer_request_t *req)
{
    int rv;
    ...
    if (sgwu_ue && cause.value == OGS_GTP_CAUSE_REQUEST_ACCEPTED) {
        bearer = sgwu_bearer_find_by_ue_ebi(sgwu_ue,
    ...
    enb = ogs_gtp_node_find_by_f_teid(
            &sgwu_self()->enb_s1u_list, enb_s1u_teid);
    if (!enb) {
        enb = ogs_gtp_node_add_by_f_teid(
            &sgwu_self()->enb_s1u_list, enb_s1u_teid, sgwu_self()->gtpu_port,
            ogs_config()->parameter.no_ipv4,
            ogs_config()->parameter.no_ipv6,
            ogs_config()->parameter.prefer_ipv4);
        ogs_assert(enb);
        ...
        rv = ogs_gtp_connect(
                sgwu_self()->gtpu_sock, sgwu_self()->gtpu_sock6, enb);
        ogs_assert(rv == OGS_OK);
    }
    ...
    if (s1u_tunnel->gnode && s1u_tunnel->gnode != enb) {
        ...
        sgwu_gtp_send_end_marker(s1u_tunnel);
    }
    ...
    /* Reset UE state */
    SGW_RESET_UE_STATE(sgwu_ue, SGW_S1U_INACTIVE);

    message.h.type = OGS_GTP_MODIFY_BEARER_RESPONSE_TYPE;
    message.h.teid = sgwu_ue->mme_s11_teid;
...
void sgwu_s11_handle_delete_session_request(ogs_gtp_xact_t *s11_xact,
    sgwu_ue_t *sgwu_ue, ogs_gtp_message_t *message)
{
    int rv;
    uint8_t cause_value = 0;
    ...
    sess = sgwu_sess_find_by_ebi(sgwu_ue, req->linked_eps_bearer_id.u8);
    ogs_assert(sess);
    ogs_debug("    MME_S11_TEID[%d] SGW_S11_TEID[%d]",
        sgwu_ue->mme_s11_teid, sgwu_ue->sgwu_s11_teid);
    ogs_debug("    SGW_S5C_TEID[0x%x] PGW_S5C_TEID[0x%x]",
...
void sgwu_s11_handle_create_bearer_response(ogs_gtp_xact_t *s11_xact,
    sgwu_ue_t *sgwu_ue, ogs_gtp_message_t *message)
{
    int rv;
    uint8_t cause_value;
    ...
    sgwu_sess_t *sess = NULL;
    sgwu_bearer_t *bearer = NULL;
    sgwu_tunnel_t *s1u_tunnel = NULL, *s5u_tunnel = NULL;
    ogs_gtp_create_bearer_response_t *req = NULL;

    ogs_gtp_f_teid_t *sgwu_s1u_teid = NULL, *enb_s1u_teid = NULL;
    ogs_gtp_f_teid_t sgwu_s5u_teid, pgw_s5u_teid;
    int len;
    ogs_gtp_uli_t uli;
    ...
    sgwu_s1u_teid = req->bearer_contexts.s4_u_sgsn_f_teid.data;
    ogs_assert(sgwu_s1u_teid);
    req->bearer_contexts.s4_u_sgsn_f_teid.presence = 0;

    /* Find the Tunnel by SGW-S1U-TEID */
    s1u_tunnel = sgwu_tunnel_find_by_teid(ntohl(sgwu_s1u_teid->teid));
    ogs_assert(s1u_tunnel);
    ...
    enb = ogs_gtp_node_find_by_f_teid(&sgwu_self()->enb_s1u_list, enb_s1u_teid);
    if (!enb) {
        enb = ogs_gtp_node_add_by_f_teid(
            &sgwu_self()->enb_s1u_list, enb_s1u_teid, sgwu_self()->gtpu_port,
            ogs_config()->parameter.no_ipv4,
    ...
    req->bearer_contexts.s5_s8_u_sgw_f_teid.data = &sgwu_s5u_teid;
...
void sgwu_s11_handle_update_bearer_response(ogs_gtp_xact_t *s11_xact,
    sgwu_ue_t *sgwu_ue, ogs_gtp_message_t *message)
{
    int rv;
    uint8_t cause_value;
    ...
    if (sgwu_ue && cause_value == OGS_GTP_CAUSE_REQUEST_ACCEPTED) {
        bearer = sgwu_bearer_find_by_ue_ebi(
                sgwu_ue, req->bearer_contexts.eps_bearer_id.u8);
    }
...
void sgwu_s11_handle_delete_bearer_response(ogs_gtp_xact_t *s11_xact,
    sgwu_ue_t *sgwu_ue, ogs_gtp_message_t *message)
{
    int rv;
    uint8_t cause_value;
...
void sgwu_s11_handle_release_access_bearers_request(ogs_gtp_xact_t *s11_xact, 
        sgwu_ue_t *sgwu_ue, ogs_gtp_release_access_bearers_request_t *req)
{
    int rv;
    ...
    SGW_SET_UE_STATE(sgwu_ue, SGW_S1U_INACTIVE);
    /* ReSet UE state to S1UE_INACTIVE */
    SGW_RESET_UE_STATE(sgwu_ue, SGW_DL_NOTI_SENT);
...
void sgwu_s11_handle_lo_dldata_notification(sgwu_bearer_t *bearer)
{
    int rv;
    ...
    xact = ogs_gtp_xact_local_create(
            sgwu_ue->gnode, &message.h, pkbuf, NULL, sgwu_ue);
...
void sgwu_s11_handle_downlink_data_notification_ack(
        ogs_gtp_xact_t *s11_xact, sgwu_ue_t *sgwu_ue,
...
void sgwu_s11_handle_create_indirect_data_forwarding_tunnel_request(
        ogs_gtp_xact_t *s11_xact, sgwu_ue_t *sgwu_ue,
        ogs_gtp_create_indirect_data_forwarding_tunnel_request_t *req)
{
    ...
    for (i = 0; req->bearer_contexts[i].presence; i++) {
        ...
        if (req->bearer_contexts[i].s1_u_enodeb_f_teid.presence) {
            ...
            tunnel = sgwu_tunnel_add(bearer,
                    OGS_GTP_F_TEID_SGW_GTP_U_FOR_DL_DATA_FORWARDING);
            ogs_assert(tunnel);

            tunnel->remote_teid = ntohl(req_teid->teid);
            enb = ogs_gtp_node_find_by_f_teid(
                    &sgwu_self()->enb_s1u_list, req_teid);
            if (!enb) {
                enb = ogs_gtp_node_add_by_f_teid(
                    &sgwu_self()->enb_s1u_list, req_teid, sgwu_self()->gtpu_port,
                    ogs_config()->parameter.no_ipv4,
                    ogs_config()->parameter.no_ipv6,
                    ogs_config()->parameter.prefer_ipv4);
                ogs_assert(enb);
            ...
            rv = ogs_gtp_sockaddr_to_f_teid(sgwu_self()->gtpu_addr,
                    sgwu_self()->gtpu_addr6, &rsp_dl_teid[i], &len);
            ogs_assert(len > 0);
        ...
        if (req->bearer_contexts[i].s12_rnc_f_teid.presence) {
            ...
            rv = ogs_gtp_sockaddr_to_f_teid(sgwu_self()->gtpu_addr,
                    sgwu_self()->gtpu_addr6, &rsp_ul_teid[i], &len);
            ogs_assert(rv == OGS_OK);
...
void sgwu_s11_handle_delete_indirect_data_forwarding_tunnel_request(
        ogs_gtp_xact_t *s11_xact, sgwu_ue_t *sgwu_ue)
{
    int rv;
    ...
    sgwu_sess_t *sess = NULL;
    sgwu_bearer_t *bearer = NULL;
    sgwu_tunnel_t *tunnel = NULL, *next_tunnel;
    ...
    while (sess) {
        ...
        while (bearer) {
            ...
            while(tunnel) {
                next_tunnel = sgwu_tunnel_next(tunnel);
...
void sgwu_s11_handle_bearer_resource_command(ogs_gtp_xact_t *s11_xact,
        sgwu_ue_t *sgwu_ue, ogs_gtp_message_t *message)
{
    int rv;
    ogs_pkbuf_t *pkbuf = NULL;
    ...
    if (cmd->linked_eps_bearer_id.presence == 0) {
        ...
        sess = sgwu_sess_find_by_ebi(sgwu_ue, cmd->linked_eps_bearer_id.u8);
        if (!sess) {
