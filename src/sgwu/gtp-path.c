...
static void _gtpv2_c_recv_cb(short when, ogs_socket_t fd, void *data)
{
     ...
     *   "Context not found".
     */
    gnode = ogs_gtp_node_find_by_addr(&sgwu_self()->pgw_s5c_list, &from);
    if (gnode) {
        e = sgwu_event_new(SGWU_EVT_S5C_MESSAGE);
        ogs_assert(e);
        e->gnode = gnode;
    } else {
        e = sgwu_event_new(SGWU_EVT_S11_MESSAGE);
        gnode = ogs_gtp_node_find_by_addr(&sgwu_self()->mme_s11_list, &from);
        if (!gnode) {
            gnode = ogs_gtp_node_add_by_addr(&sgwu_self()->mme_s11_list, &from);
            ogs_assert(gnode);
            gnode->sock = data;
        }
        ogs_assert(e);
...
int sgwu_gtp_open(void)
{
...
#if 0
    ogs_list_for_each(&sgwu_self()->gtpc_list, node) {
    ...
    ogs_list_for_each(&sgwu_self()->gtpc_list6, node) {
        sock = ogs_gtp_server(node);
        ogs_assert(sock);
    ...
    sgwu_self()->gtpc_sock = ogs_socknode_sock_first(&sgwu_self()->gtpc_list);
    if (sgwu_self()->gtpc_sock)
        sgwu_self()->gtpc_addr = &sgwu_self()->gtpc_sock->local_addr;

    sgwu_self()->gtpc_sock6 = ogs_socknode_sock_first(&sgwu_self()->gtpc_list6);
    if (sgwu_self()->gtpc_sock6)
        sgwu_self()->gtpc_addr6 = &sgwu_self()->gtpc_sock6->local_addr;

    ogs_assert(sgwu_self()->gtpc_addr || sgwu_self()->gtpc_addr6);
#endif
...
{
#if 0
    ogs_socknode_remove_all(&sgwu_self()->gtpc_list);
    ogs_socknode_remove_all(&sgwu_self()->gtpc_list6);
#endif
